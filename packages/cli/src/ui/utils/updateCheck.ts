  ...
  try {
    ...
    if (isNightly) {
      ...
      if (bestUpdate && semver.gt(bestUpdate.latest, currentVersion)) {
        const message = `A new version of LLM CLI is available! ${currentVersion} → ${bestUpdate.latest}`;
        return {
          message,
      ...
      if (updateInfo && semver.gt(updateInfo.latest, currentVersion)) {
        const message = `LLM CLI update available! ${currentVersion} → ${updateInfo.latest}`;
        return {
          message,
